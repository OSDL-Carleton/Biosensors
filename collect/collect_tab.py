...
def collect_tab(tab_collect, root):
    ...
    vscroll = tk.Scrollbar(container, orient="vertical", command=canvas.yview, width=20) # Wider scrollbar
...
def save_excel_file(
    ...
    for i in range(max_length):
        ...
        for dac1_voltage in data_by_dac1:
            i4_value = (
                data_by_dac1[dac1_voltage]["currents4"][i]
                if i < len(data_by_dac1[dac1_voltage]["currents4"])
                else ""
            )
            row_data.append(i4_value)
...
def create_plot_frame(parent):
    ...
    # Keep figsize relatively small to fit 800x400 screens.
    # (5, 3) is 500x300 pixels at 100 DPI.
    fig, ax = plt.subplots(figsize=(5, 3)) # Or even (5, 2.5) if height is still an issue

    # --- KEY CHANGE: Adjust subplot parameters to reduce right margin ---
    # Experiment with these values.
    # A smaller 'right' value will reduce the right margin.
    # A larger 'left' value will reduce the left margin.
    # A smaller 'top' value reduces top margin, 'bottom' for bottom.
    # Values are between 0 and 1, representing a fraction of the figure's width/height.
    fig.subplots_adjust(left=0.15, right=0.95, top=0.9, bottom=0.15)
    # Explanation:
    # left=0.15: Leaves 15% of figure width on the left (for y-axis labels)
    # right=0.95: Sets the right edge of the plot area at 95% of figure width (reducing white space on right)
    # top=0.9: Sets the top edge of the plot area at 90% of figure height (for title if needed)
    # bottom=0.15: Leaves 15% of figure height on the bottom (for x-axis labels)
    ...
    # Pack the toolbar first to ensure it's at the bottom
    toolbar.pack(side=tk.BOTTOM, fill=tk.X, expand=False)

    # Then pack the canvas to take up all *remaining* space
